...
public class DirectOperatorTest {
  ...
  public void testProxyWrite()
    ...
    Set<DirectAttributeFamilyDescriptor> families = direct
        .getFamiliesForAttribute(target);

    Set<DirectAttributeFamilyDescriptor> proxiedFamilies = direct
        .getFamiliesForAttribute(source);

    assertEquals(
        families.stream()
          .map(a -> "proxy::" + a.getDesc().getName()
              + "::" + a.getDesc().getName())
          .collect(Collectors.toList()),
        proxiedFamilies.stream()
          .map(a -> a.getDesc().getName())
          .collect(Collectors.toList()));

    // verify that writing to attribute event.abc ends up as _e.abc
    CountDownLatch latch = new CountDownLatch(2);
    proxiedFamilies.iterator()
        .next()
    ...
    direct.getWriter(source).get().write(StreamElement.update(
        proxied,
  ...
  private void testProxyObserveWithAttributeName(String name)
    ...
    EntityDescriptor proxied = repo.findEntity("proxied").get();
    AttributeDescriptor<?> source = proxied.findAttribute("event.*").get();
    CommitLogReader reader = direct.getFamiliesForAttribute(source)
        .stream()
  ...
  public void testReplicationWriteObserve() throws InterruptedException {
    ...
    Optional<KeyValue<Object>> kv = direct.getFamiliesForAttribute(armed)
        .stream()
        .filter(af -> af.getDesc().getAccess().canRandomRead())
        .findAny()
        .flatMap(af -> af.getRandomAccessReader())
  ...
  private void testReplicationWriteObserveInternal(
    ...
    // observe stream
    CommitLogReader reader = direct.getFamiliesForAttribute(armed)
        .stream()
        .filter(af -> af.getDesc().getAccess().canReadCommitLog())
        .findAny()
        .flatMap(af -> af.getCommitLogReader())
  ...
  public void testWriteIntoReplicatedProxyAttribute()
    ...
    CountDownLatch latch = new CountDownLatch(2);
    CommitLogReader reader = direct.getFamiliesForAttribute(data)
        .stream()
        .filter(af -> af.getDesc().getAccess().canReadCommitLog())
        .findAny()
        .flatMap(af -> af.getCommitLogReader())
  ...
  public void testRandomReadFromReplicatedProxyAttribute()
    ...
    Optional<RandomAccessReader> reader = direct.getFamiliesForAttribute(data)
        .stream()
        .filter(af -> af.getDesc().getAccess().canRandomRead())
        .findAny()
        .flatMap(af -> af.getRandomAccessReader());
  ...
  public void testRandomReadFromReplicatedProxyAttributeDirect()
    ...
    Optional<RandomAccessReader> reader = direct.getFamiliesForAttribute(event)
        .stream()
        .filter(af -> af.getDesc().getAccess().canRandomRead())
        .findAny()
        .flatMap(af -> af.getRandomAccessReader());
    ...
    reader = direct.getFamiliesForAttribute(raw)
        .stream()
        .filter(af -> af.getDesc().getAccess().canRandomRead())
        .findAny()
  ...
  public void testApplicationOfProxyTransformOnReplicatedData()
    ...
    Optional<RandomAccessReader> reader = direct
  ...
  public void testApplicationOfProxyTransformOnReplicatedDataWithTransform()
    ...
    Optional<RandomAccessReader> reader = direct.getFamiliesForAttribute(raw)
        .stream()
        .filter(af -> af.getDesc().getAccess().canRandomRead())
        .findAny()
        .flatMap(DirectAttributeFamilyDescriptor::getRandomAccessReader);
  ...
  void testFullReplicationWithEntities(
    ...
    direct.getWriter(wildcardSecond).get().write(
        StreamElement.deleteWildcard(
            first, wildcardSecond, "uuid", "key", wildcardSecond.toAttributePrefix(),
            now + 1),
        (succ, exc) -> { });

    reader = direct.getFamiliesForAttribute(wildcardFirst)
        .stream()
        .filter(af -> af.getDesc().getAccess().canRandomRead())
        .findAny()
        .flatMap(DirectAttributeFamilyDescriptor::getRandomAccessReader);
  ...
  public void testObserveReplicatedWithProxy() throws InterruptedException {
    ...
    latch.await();
    assertFalse(
        direct.getFamiliesForAttribute(dataWrite)
          .stream()
  ...
  public void testIncomingReplicationDoesntCycle() throws InterruptedException {
    ...
    TransformationRunner.runTransformations(repo, direct);
    CommitLogReader reader = direct.getFamiliesForAttribute(status)
        .stream()
        .filter(af -> af.getDesc().getAccess().canReadCommitLog())
        .findAny()
        .flatMap(DirectAttributeFamilyDescriptor::getCommitLogReader)
    ...
    latch.await();
    RandomAccessReader localReader = direct.getFamiliesForAttribute(statusWrite)
        .stream()
        .filter(af -> af.getDesc().getType() == StorageType.PRIMARY)
        .findAny()
        .flatMap(DirectAttributeFamilyDescriptor::getRandomAccessReader)
  ...
  public void testWildcardDeleteReplication() throws InterruptedException {
    ...
    DirectAttributeFamilyDescriptor outputFamily = direct.getAllFamilies()
        .filter(af -> af.getDesc().getName().equals(
            "replication_dummy2-replication_source"))
        .findAny()
        .orElseThrow(() -> new IllegalStateException(
