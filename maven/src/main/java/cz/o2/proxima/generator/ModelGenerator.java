...
public class ModelGenerator {
  ...
  public ModelGenerator(
          String javaPackage, String className, File configPath, File outputPath) {
  ...
  ModelGenerator(
    ...
    if (validate) {
      ...
      if (!this.outputPath.isAbsolute()) {
        throw new IllegalArgumentException(String.format("Output path must be absolute [%s]", outputPath));
      }
    }
  }
  ...
  public void generate() throws Exception {
    ...
    if (!output.exists() && !output.mkdirs()) {
      throw new RuntimeException(
              String.format(
          "Failed to create directories for [%s]", outputPath.getAbsolutePath()));
    }
                          ...
                          .orElseThrow(
                              () ->
                                  new IllegalStateException(String.format(
                                      "Unable to retrieve serializer factory for scheme %s. Looks like missing dependency for maven plugin.", attr.getSchemeUri().getScheme())));
